...
namespace nwindow
{
  bool GLWindow::init(int width, int height, const std::string& title)
  {
    ...
    mSceneView->NodeEditorInitialize();
    //*(mSceneView->current_code)="init";//[0] = "a";

    //mSceneView->txt;//[0] = "a";
   //memcpy(mSceneView->current_code,"aa",2);
 ...
 //  }
  ...
  void GLWindow::handle_input()
  {
        ...
        if (currentEvent.type == SDL_KEYDOWN && currentEvent.key.keysym.sym == SDLK_ESCAPE) {
            //quit = true;
                      on_close();
            printf("QUIT");
            break;
